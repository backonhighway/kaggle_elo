...
class OofFe3:
    ...
    def _do_encode(from_ts, to_ts):
        ...
        for col in cat_col:
            ...
            count_col = col + "_cnt"
            ts_source[count_col] = ts_source.groupby(col)["target"].transform("count")
    ...
    def _do_agg(self, encoded_ts):
        cat_col = ["merchant_id", "subsector_id"]
        aggs = {}
        tmp_df = None
        for c in cat_col:
            g = encoded_ts.groupby(["card_id", c])[c + "_target_encode"].mean().reset_index()
            g.columns = ["card_id", c] + [c + "_te"]
            if tmp_df is None:
                tmp_df = g
            else:
                tmp_df = pd.merge(tmp_df, g, on="card_id", how="left")
            aggs[c + "_te"] = ["mean"]

        ret_df = tmp_df.groupby("card_id").agg(aggs).reset_index()
