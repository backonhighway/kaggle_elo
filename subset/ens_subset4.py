...
class GoldenLr:
    ...
    def doit_fast(self):
        ...
        bin_files = ["subset_exp_" + str(idx) for idx in range(10)]
        bin_files = [(f, "bin"+f) for f in bin_files]
        no_out_files = ["subset_exp_" + str(idx) for idx in range(10)]
    ...
    def make_files(self, base_files, bin_files=None, no_out_files=None):
        ...
        if bin_files is not None:
            for f in bin_files:
                train, test = self._add_file(f[0], f[1], "../output/subset_bin1/", train, test)
        if no_out_files is not None:
            for f in no_out_files:
                train, test = self._add_file(f[0], f[1], "../output/subset_no_out1/", train, test)
        return train, test
